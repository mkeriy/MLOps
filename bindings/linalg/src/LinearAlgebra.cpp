...
std::vector<std::vector<double>> 
LinearAlgebra::cosineSimilarity(const std::vector<std::vector<double>> &a, 
                                const std::vector<std::vector<double>> &b)
{
  size_t aRows = a.size(); 
  size_t aCols = a[0].size();
  ...
  if (aCols != bCols) 
  {
    throw std::runtime_error("Sizes of the vectors to calculate the distance " 
                              "between them must be equal\n"
                              "Given " + std::to_string(aCols) + " and " + std::to_string(bCols));
  }

  std::vector<std::vector<double>> cosineSim(aRows, std::vector<double>(bRows));

  for (size_t i = 0; i < aRows; ++i)
  {
    for (size_t j = 0; j < bRows; ++j)
    {
      aNorm = cblas_dnrm2(aCols, &a[i][0], 1);
      bNorm = cblas_dnrm2(bCols, &b[j][0], 1); 
      cosineSim[i][j] = dotBlas(a[i], b[j]) / (aNorm * bNorm);
    }
  }
