...
def try_cosine_sim(matrix_size: int) -> None:
    matrix_a = np.random.rand(matrix_size, matrix_size)
    matrix_b = np.random.rand(matrix_size, matrix_size)
    ...
    list_b = matrix_b.tolist()
    print(
        "Cos (Python numpy + sklearn), size={0}x{0}: {1} seconds\n".format(
            matrix_size, test_timings(cosine_similarity, np.array(list_a), np.array(list_b))
        )
    )
    
    print(
        "Cos (C++ BLAS), size={0}x{0}: {1} seconds\n".format(
            matrix_size, test_timings(linalg.LinearAlgebra.cosineSimilarity, list_a, list_b)
        )
    )
